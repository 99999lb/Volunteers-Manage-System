...
public class ArticleDao {
    ...
    public ArrayList<Article> queryPacs(){
        ArrayList<Article> list =new ArrayList<>();
        Article p=null;
        String sql="select * from pac";
        ResultSet rs= SQLHelper.executeQuery(sql);
        try{
            while (rs.next()){
                p=new Article(rs.getString(1),rs.getString(2),rs.getString(3),Integer.parseInt(rs.getString(4)));
    ...
    public ArrayList<Article> queryPacsByTid(String tid){
        ArrayList<Article> list =new ArrayList<>();
        Article p=null;
        String sql="select * from pac where NID='"+tid+"'";
        ResultSet rs= SQLHelper.executeQuery(sql);
        try{
            while (rs.next()){
                p=new Article(rs.getString(1),rs.getString(2),rs.getString(3),Integer.parseInt(rs.getString(4)));
    ...
    public Boolean addPacs(String aid,String title,String tText) throws SQLException {
        Boolean flag=false;
        int rs= SQLHelper.executeUpdate("insert into pac(aid,Title,Ttext) values('"+aid+"','"+title+"','"+tText+"')");
        if (rs!=0)
            flag=true;
        return flag;
    }
    ...
    public Boolean deletePac(String tid) throws SQLException {
        Boolean flag=false;
        int rs= SQLHelper.executeUpdate("delete from pac where NID='"+tid+"'");
        if (rs!=0)
            flag=true;
        return flag;
    }
